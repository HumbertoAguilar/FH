...
var topWall = Bodies.rectangle(750, -20, 1200, 20, { isStatic: true,
render:{
sprite:{
  texture:'palos.png'
}
...
var bottomWall = Bodies.rectangle(750,400, 1200, 20, { isStatic: true,
render:{
  sprite:{
    texture:'palos.png',
    xScale:4,
...
var porteriader=Bodies.rectangle(1130,200,130,20,{isStatic:true,
render:{
  sprite:{
...
var car = Bodies.rectangle(1200, 100,60,95,{angle:0,
render:{
sprite:{
...
function setup(){
  var canvas = createCanvas(1500,150);
marcador=createSprite(750,40, 650,80)
marcador.shapeColor="black"
marco=createSprite(755,35)
marco.addImage(marcoimg)
marco.scale=0.25
marco2=createSprite(685,35)
marco2.addImage(marcoimg)
marco2.scale=0.25
marcador2=createSprite(750,100,150,50)
marcador2.shapeColor="black"
player1=createSprite(550,40)
player1.scale=0.8
player1.addImage(player1img)
player2=createSprite(950,40)
player2.scale=0.8
...
//int=createSprite(1360,50)
  ...
  //int.scale=.22
buton=createButton('| |')
buton.position(1270,660)
  buton2=createButton('? ')
  buton2.position(1400,660);
  flechas=createSprite(1270,80)
flechas.addImage(flechasimg)
flechas.scale=2.5
flechas.visible=false
m=createSprite(1030,20)
m.addImage(Mimg)
m.scale=2.5
m.visible=false
...
if(keyDown(38)&&car.position.y>400){
...
if(keyDown("w")&&car2.position.y>400){
...
if(keyWentDown('r')&&ball.position.x<car2.position.x+100&&ball.position.y>400){
  Body.applyForce(
    ball,
...
if(ball.position.x<105&&ball.position.y>200&&ball.position.y<500){
  score2+=+1
  goalSound.play()
  gameState+=+1
}
...
if(ball.position.x>1360&&ball.position.y>200&&ball.position.y<500){
score+=+1
goalSound.play()
